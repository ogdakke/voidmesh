...
describe("blurParamToKawaseParams", () => {
  ...
  test("returns 1 level for small blur values", () => {
    const result = blurParamToKawaseParams(0.05);
  ...
  test("levelsLow increases monotonically with blur value", () => {
    const values = [0.5, 2, 5, 10, 15, 21, 27];
    let prevLevels = 0;
  ...
  test("maximum blur value uses high level count", () => {
    const result = blurParamToKawaseParams(30);
  ...
  test("blend zone activates near breakpoint boundaries", () => {
    // Near the boundary between segments (e.g., just before value 1.0)
    const nearBoundary = blurParamToKawaseParams(0.95);
  ...
  test("no blending in the middle of a segment", () => {
    // Middle of the 3-7 segment (level 4)
    const mid = blurParamToKawaseParams(4);
    expect(mid.blendFactor).toBe(0);
    expect(mid.levelsLow).toBe(mid.levelsHigh);
  });

  test("continuity at breakpoint transitions", () => {
    // Just before a breakpoint, blendFactor should be near 1 with levelsHigh = nextLevel.
    // Just after, blendFactor should be 0 with levelsLow = nextLevel.
    // The effective visual level should be continuous.
    const breakpoints = [0.1, 1, 3, 7, 12, 18, 24];
    for (const bp of breakpoints) {
      const before = blurParamToKawaseParams(bp - 0.001);
      const after = blurParamToKawaseParams(bp + 0.001);

      // The "effective" level from before (dominated by levelsHigh due to blendFactor near 1)
      // should match the level from after (levelsLow with blendFactor near 0)
