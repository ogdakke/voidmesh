...
function createHarness() {
  ...
  const viewportAnimation = {
    animateTo,
  } as unknown as ViewportAnimationController;
  const actionLayer = {
    getEntityOffset: vi.fn<ActionLayerController["getEntityOffset"]>(() => ({
      x: 0,
      y: 0,
    })),
...
describe("CanvasInteractionService", () => {
  it("initializes the viewport from surface metrics", async () => {
    const { store, service } = createHarness();
    let initialized = false;
    void store.whenViewportInitialized().then(() => {
      initialized = true;
    });
    service.initializeViewport({ width: 800, height: 600, dpr: 2 });
    await store.whenViewportInitialized();
    expect(store.getViewport()).toEqual({
      offset: { x: -800, y: -600 },
      zoom: 1,
    });
