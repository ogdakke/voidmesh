...
function createCanvasEditAccess(initialValue: boolean) {
  let canEdit = initialValue;
  return {
    get: () => canEdit,
    set: (value: boolean) => {
      canEdit = value;
...
export function CanvasProvider({
  access = LOCAL_CANVAS_ACCESS,
  children,
}: {
  access?: CanvasAccessPolicy;
  children: ReactNode;
}) {
  ...
  useEffect(() => {
    ...
    gameLoop.setReadOnly(!access.canEdit);
    return () => gameLoop.setReadOnly(false);
  }, [access]);
  const [mediaAccess] = useState(() => createCanvasEditAccess(access.canEdit));
  useEffect(() => {
    mediaAccess.set(access.canEdit);
  }, [access.canEdit, mediaAccess]);
  ...
  const [media] = useState(() => createCanvasMediaService(canvasStore, mediaAccess.get));
  ...
  const rejectReadOnlyImport = (): Promise<never> =>
    Promise.reject(new Error("Viewers cannot modify this workspace"));
  const [commands] = useState<CanvasCommands>(() => ({
    ...
    addEntity: (...args) => {
      if (!accessRef.current.canEdit) throw new Error("Viewers cannot add media");
      return commandsImplRef.current.addEntity(...args);
    },
    updateEntity: (...args) => {
      if (accessRef.current.canEdit) commandsImplRef.current.updateEntity(...args);
    },
    removeEntity: (...args) => {
      if (accessRef.current.canEdit) commandsImplRef.current.removeEntity(...args);
    },
    clearWorkspace: () => {
      if (accessRef.current.canEdit) commandsImplRef.current.clearWorkspace();
    },
    selectEntity: (...args) => commandsImplRef.current.selectEntity(...args),
    moveEntity: (...args) => {
      if (accessRef.current.canEdit) commandsImplRef.current.moveEntity(...args);
    },
    duplicateEntities: () =>
      accessRef.current.canEdit ? commandsImplRef.current.duplicateEntities() : Promise.resolve([]),
    updateSelectedEntityParams: (...args) => {
      if (accessRef.current.canEdit) commandsImplRef.current.updateSelectedEntityParams(...args);
    },
    setSelectedEntityTimeAutoPlay: (...args) => {
      if (accessRef.current.canEdit) commandsImplRef.current.setSelectedEntityTimeAutoPlay(...args);
    },
    syncSelectedEntityTimes: () => {
      if (accessRef.current.canEdit) commandsImplRef.current.syncSelectedEntityTimes();
    },
    changeShaderType: (...args) => {
      if (accessRef.current.canEdit) commandsImplRef.current.changeShaderType(...args);
    },
    changeDitheringKind: (...args) => {
      if (accessRef.current.canEdit) commandsImplRef.current.changeDitheringKind(...args);
    },
    changeAsciiKind: (...args) => {
      if (accessRef.current.canEdit) commandsImplRef.current.changeAsciiKind(...args);
    },
    setAsciiInvert: (...args) => {
      if (accessRef.current.canEdit) commandsImplRef.current.setAsciiInvert(...args);
    },
    changeGlassKind: (...args) => {
      if (accessRef.current.canEdit) commandsImplRef.current.changeGlassKind(...args);
    },
    changeGlitchKind: (...args) => {
      if (accessRef.current.canEdit) commandsImplRef.current.changeGlitchKind(...args);
    },
    changePalette: (...args) => {
      if (accessRef.current.canEdit) commandsImplRef.current.changePalette(...args);
    },
    renamePalette: (...args) => {
      if (accessRef.current.canEdit) commandsImplRef.current.renamePalette(...args);
    },
    uploadPalette: (...args) =>
      accessRef.current.canEdit
        ? commandsImplRef.current.uploadPalette(...args)
        : Promise.resolve(),
    deletePalette: (...args) => {
      if (accessRef.current.canEdit) commandsImplRef.current.deletePalette(...args);
    },
    setShowOriginal: (...args) => {
      if (accessRef.current.canEdit) commandsImplRef.current.setShowOriginal(...args);
    },
    toggleShowOriginal: () => {
      if (accessRef.current.canEdit) commandsImplRef.current.toggleShowOriginal();
    },
    setPreserveColors: (...args) => {
      if (accessRef.current.canEdit) commandsImplRef.current.setPreserveColors(...args);
    },
    togglePreserveColors: () => {
      if (accessRef.current.canEdit) commandsImplRef.current.togglePreserveColors();
    },
    setReversePalette: (...args) => {
      if (accessRef.current.canEdit) commandsImplRef.current.setReversePalette(...args);
    },
    toggleReversePalette: () => {
      if (accessRef.current.canEdit) commandsImplRef.current.toggleReversePalette();
    },
    deleteSelection: (...args) => {
      if (accessRef.current.canEdit) commandsImplRef.current.deleteSelection(...args);
    },
    copySelectionImage: (...args) =>
      accessRef.current.canExportWorkspace
        ? commandsImplRef.current.copySelectionImage(...args)
        : Promise.resolve(),
    copySelectionEffects: () => {
      if (accessRef.current.canExportWorkspace) commandsImplRef.current.copySelectionEffects();
    },
    pasteEffects: () =>
      accessRef.current.canEdit ? commandsImplRef.current.pasteEffects() : Promise.resolve(),
    resetSelectionToDefaults: () => {
      if (accessRef.current.canEdit) commandsImplRef.current.resetSelectionToDefaults();
    },
    ...
    changeSize: (...args) => {
      if (accessRef.current.canEdit) commandsImplRef.current.changeSize(...args);
    },
    copySelectedEntityToClipboard: () =>
      accessRef.current.canExportWorkspace
        ? commandsImplRef.current.copySelectedEntityToClipboard()
    ...
    serializeCanvas: () =>
      accessRef.current.canExportWorkspace
        ? commandsImplRef.current.serializeCanvas()
        : Promise.resolve(null),
    deserializeCanvas: (...args) =>
      accessRef.current.canEdit
        ? commandsImplRef.current.deserializeCanvas(...args)
        : rejectReadOnlyImport(),
    applyUrlState: (...args) => {
      if (accessRef.current.canEdit) commandsImplRef.current.applyUrlState(...args);
    },
    applyEffectsToSelection: (...args) => {
      if (accessRef.current.canEdit) commandsImplRef.current.applyEffectsToSelection(...args);
    },
  ...
  return (
    <CanvasAccessContext value={access}>
