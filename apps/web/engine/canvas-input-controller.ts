...
export class CanvasInputController {
  ...
  setReadOnly(readOnly: boolean): void {
    ...
    this.#touchState.isDraggingEntity = false;
  ...
  handlePointerDown(screenPoint: Point, shiftKey: boolean = false): void {
    ...
    if (entityId) {
      ...
      const target = this.#selection.choosePointerDownEntityTarget(entityId, shiftKey, state);
      this.#entityDrag.setTarget(this.#readOnly ? null : target);
    ...
    } else if (!this.#readOnly && isInMultiSelectBounds && !shiftKey) {
  ...
  handleTouchStart(touches: Point[], eventTime?: number): void {
    ...
    if (touches.length === 1) {
      ...
      if (this.#touchState.isDraggingEntity && !this.#readOnly) {
  ...
  #handleTouchTap(): void {
    ...
    if (this.isInMultiSelectMode()) {
      ...
      if (entityId) {
        ...
        } else if (this.#inputState.pointerDownWasSelected && !this.#readOnly) {
