...
- Bound collaborative asset requests and serialize whole-asset sends per peer. Require a receiver acknowledgement after restore and hash verification before sending the next Blob; reset a receiver watchdog on progress so incomplete sends, mobile page suspension, negative acknowledgements, and departed peers release or retry pending work rather than strand placeholders.
