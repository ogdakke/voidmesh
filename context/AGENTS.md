...
- Bound collaborative asset requests, serialize payload sends per peer, and pipeline only a small fixed window of outstanding restore/hash-verification acknowledgements so TURN RTT does not impose stop-and-wait latency per Blob. Reset a receiver watchdog on progress so incomplete sends, mobile page suspension, negative acknowledgements, and departed peers release or retry pending work rather than strand placeholders.
