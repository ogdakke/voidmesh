...
- Replace Trystero's whole-payload allocation/reassembly with application-owned streaming chunks, resumable offsets, incremental hashing, and multi-peer source selection for large videos. The current bounded, acknowledged queue provides backpressure between whole assets but cannot resume within one Blob.
