...
- `game-loop.ts` (~73KB) — `GameLoop` class. `requestAnimationFrame` loop, pointer/touch events, entity hit detection, drag-select, multi-touch pinch/pan, long-press entity drag, space+drag canvas panning.
...
- Snapshot types (`ViewportSnapshot`, `SelectionSnapshot`, `PlaybackSnapshot`, `DragSnapshot`) isolate subscription scopes — sidebar components don't re-render on viewport pan.
...
- Disintegration controller is ticked per frame alongside drag visuals; returns `true` while animations are active to keep the render loop running.
