...
- `game-loop.ts` (~73KB) — `GameLoop` class. `requestAnimationFrame` loop, pointer/touch events, entity hit detection, drag-select, multi-touch pinch/pan, long-press entity drag.
...
- `CanvasState` uses version counters (`version`, `viewportVersion`, `selectionVersion`, `playbackVersion`) for selective React subscriptions.
- Snapshot types (`ViewportSnapshot`, `SelectionSnapshot`, `PlaybackSnapshot`) isolate subscription scopes — sidebar components don't re-render on viewport pan.
