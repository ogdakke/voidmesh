...
- Original video playback stays zero-copy on external textures. Re-import HTML video sources as required by WebGPU expiry, but reuse external composition bindings whenever the browser revives the same texture identity; do not copy video frames into renderer-owned RGBA textures.
