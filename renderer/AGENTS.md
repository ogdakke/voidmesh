...
- Selection and debug flags are stored in the versioned full-scene instance payload, so selection/debug viewport frames remain CPU-constant. Large drags that target the full selection apply one uniform offset/scale without rebuilding or uploading instances; committed geometry rebuilds once on release. A single selected label is encoded in a dedicated scene-overlay pass after base and action-layer entity composition, so it ignores entity z-order without splitting instanced draws; canvas-wide lensing and progressive blur still apply afterward. Multi-selection rectangles remain shader-rendered. Action-layer fade/blur, partial-target drag visuals, drag-select, callouts, continuous shaders, and heterogeneous media must use normal preparation. The batch may submit offscreen instances after admission; hardware clipping keeps the pan path CPU-constant.
