...
- A persistent static-image composition plan has no entity-count floor, but requires at least 25% of the scene visible. Identical effect/source scenes use the representative fast path. Mixed assets/effects may persist up to 256 consecutive texture runs, preserving exact z-order while avoiding per-pan visibility, texture-binding, and instance reconstruction; continuous shaders, external media, or excessively fragmented scenes stay on normal preparation. Cache hits skip spatial queries and instance uploads; entity/geometry/selection versions, loss of an exact cached texture, or any normal composition write invalidate the plan. Zoom motion reuses the current mixed plan because projection is uniform-driven; after motion stops, visible preparation converges bounded LOD work before the final texture-run plan is rebuilt once.
- When at most 32 existing image entities change without reordering a persistent static plan, resolve only those dirty IDs, patch their 32-byte instance records, and split or merge their local texture runs. Assets may differ. Do not rescan structural shader equality or rebuild/upload the full scene for small parameter edits; membership, reordering, incompatible media, excessive run fragmentation, or texture eviction falls back to conservative full admission.
