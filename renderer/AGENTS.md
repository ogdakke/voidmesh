...
- Regular-texture tier changes crossfade their current/previous resident textures for a bounded interval. A raw `GPUTexture` reference is not ownership: acquire a verified short-lived cache pin before binding the previous tier, exclude pinned entries from explicit release/LRU eviction, and cancel the fade if residency was lost while offscreen. Batch adjacent entities by the exact texture pair and use explicit-level sampling inside per-instance WGSL branches; external video textures never enter this path.
