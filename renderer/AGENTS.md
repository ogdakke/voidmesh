...
- `composition-pass.ts` + `composition-instanced.wgsl` — Final canvas composition. Adjacent regular-texture entities sharing the same texture use instanced draws; large shared-asset overview scenes persist homogeneous or bounded mixed-texture instance plans.
- `entity-draw-item-preparer.ts` — Visibility/LOD preparation plus conservative admission of versioned full-scene batches for large static-image scenes.
...
- A full-scene batch is admitted only for 16k+ static image entities with at least 25% visible. Identical effect/source scenes use the representative fast path. Shared-asset mixed-effect scenes may persist up to 256 consecutive texture runs, preserving exact z-order while avoiding per-pan visibility, texture-binding, and instance reconstruction; distinct-asset, continuous, external-media, or excessively fragmented scenes stay on normal preparation. Cache hits skip spatial queries and instance uploads; entity/geometry/selection versions, texture-cache revisions, or any normal composition write invalidate the plan. Zoom motion reuses the current mixed plan because projection is uniform-driven; after motion stops, visible preparation converges bounded LOD work before the final texture-run plan is rebuilt once.
- When at most 32 existing entities change without reordering a persistent shared-asset scene, resolve only those dirty IDs, patch their 32-byte instance records, and split or merge their local texture runs. Do not rescan structural shader equality or rebuild/upload the full scene for small parameter edits; membership, reordering, incompatible media, excessive run fragmentation, or texture eviction falls back to conservative full admission.
- Selection and debug flags are stored in the versioned full-scene instance payload, so committed selection/debug viewport frames remain CPU-constant. During drag selection, the immutable gesture-start flags combine with replace/add/subtract uniforms and rotated entity AABBs in the instanced vertex shader; locked entities retain CPU selection semantics. Large drags that target the full selection likewise apply one uniform offset/scale, and committed state rebuilds once on release. A single selected label is encoded in a dedicated scene-overlay pass after base and action-layer entity composition, so it ignores entity z-order without splitting instanced draws; canvas-wide lensing and progressive blur still apply afterward. Multi-selection rectangles remain shader-rendered. Action-layer fade/blur, partial-target drag visuals, callouts, continuous shaders, and heterogeneous media must use normal preparation. The batch may submit offscreen instances after admission; hardware clipping keeps the pan path CPU-constant.
